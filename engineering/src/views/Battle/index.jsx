  ...
  return (
    <div>
      ...
      <div className='flex justify-center'>
        <input className='border-solid  border-2 border-light-blue-500 h-14 w-8/12 p-2' onChange={(e) => props.onChangInput(e, keyValue)} type="text" name={keyValue} placeholder='gthub username' />
        <button className='w-3/12 h-14 bg-gray-200 ml-4'
          onKeyDown={(e) => {
            if (e.keyCode == 3) {
          ...
          disabled={ props[keyValue]?false:true}>Submit</button>
      </div>
    </div>
  )
}
      ...
      <div>
        ...
        <div className='flex flex-wrap justify-center'>
          <div className='xl:flex-1 items-end'>
            <h4 className='text-right p-10 text-2xl'>
                            Enter two Github
            </h4>
            <span className='flex justify-end p-10 text-9xl'>
              <i className='fa fa-users _2jxN-VXSysaUNXEOXoVAUp' style={{ color: 'rgb(255, 191, 116)' }}></i>
            </span>
          </div>
          <div className='xl:flex-2 items-end'>
            <h4 className='text-center p-10 text-2xl'>Battle</h4>
            <span className='flex justify-end p-10 text-9xl'>
              <i className='fa fa-fighter-jet _2jxN-VXSysaUNXEOXoVAUp' style={{ color: 'gray' }}></i>
            </span>
          </div>
        ...
        <div className='flex flex-wrap pb-4'>
          <div className='xl:w-1/2 w-full'>
            {
              !oneNameflag && <Inputs title='Player One' keyValue='oneName' onChangInput={this.changInput} onClickAdd={this.handleAdd} oneName={ oneName} />
            }
            {
              oneName && oneNameflag && <Card keys='oneName' keyValue={oneName} handeldel={this.handeldels} />
            }
          </div>
          <div className='xl:w-1/2 w-full'>
            {
              !twoNameflag && <Inputs title="Player Two" keyValue='twoName' onChangInput={this.changInput} onClickAdd={this.handleAdd} twoName={twoName} />
            }
            {
              twoName && twoNameflag && <Card keys='twoName' keyValue={twoName} handeldel={this.handeldels} />
            }
          </div>
        </div>
        {
          oneNameflag && twoNameflag && <button onClick={this.handelBattle} className="rounded bg-gray-300 px-8 pt-4 py-4 m-auto flex mt-4">Battle</button>
        }
